  ...
  const uploadFile = async () => {
    ...
    try {
      ...
      uploadTask.on(
        ...
        (err) => {
          console.error(err);
          setErrorMsg('Upload failed. Check your Firebase config.');
          setState('error');
        ...
        async () => {
          try {
            ...
            // Also persist to localStorage for gallery
            const meta = {
              id:          docRef.id,
              fileName:    selectedFile.name,
              fileType:    selectedFile.type,
      ...
      console.error(e);
      setErrorMsg('Upload failed. Check Firebase config.');
      setState('error');
      addToast('Upload failed', 'error');
    }
  };
        ...
        {isError && (
          <div className="flex flex-col items-center gap-4 page-enter">
            ...
            <div className="text-center">
              ...
              <p className={`text-xs ${textMuted} max-w-xs`}>{errorMsg}</p>
            </div>
            <button
              onClick={(e) => { e.stopPropagation(); reset(); }}
